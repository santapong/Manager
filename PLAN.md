...
- 2026-06-20 — AI assistant built through Wave 2c and the **MCP connector (2d) deferred**. Shipped: BYO encrypted key + settings (2a), streaming chat with read-only RLS-scoped tools (2b), and write actions via **propose→confirm→apply** where the model never mutates — write tools emit proposals and the only write path is a user-confirmed, RLS-scoped Server Action (2c). Deferred with rationale: **Inngest** isn't needed yet (chat turns are bounded and stream within one request); the **MCP connector** requires the Manager MCP server at a public HTTPS URL with per-workspace PAT auth (it's stdio-only today) and the beta connector can't be exercised against localhost/preview — in-process tools already deliver the capability portably and are the recommended path (`docs/research/ai/03-mcp-and-tools.md`). Built by sequential Opus squads; every increment passed typecheck/lint/build before commit.
- 2026-06-20 — AI assistant direction set (ADR 0002 + `docs/research/ai/`), researched by an Opus team. Wrap the **official `@anthropic-ai/sdk` behind an `AIService` port** in `packages/ai` (not the Vercel AI SDK — avoid provider lock-in on a self-host-first product); stream from a **Node Route Handler**, push long agentic loops to Inngest; default **`claude-opus-4-8`** with adaptive thinking + effort, prompt-cache the system+tools prefix as the cost lever. **BYO per-workspace API key** stored **AES-256-GCM-encrypted** (`node:crypto`, new `AI_ENCRYPTION_KEY` env, no new dep) in a new RLS-isolated `workspace_ai_keys` table with a write-only Settings page — this is the project's first encryption-at-rest primitive. Assistant actions via **in-process custom tools** for the MVP (RLS-safe, portable), with the beta **MCP connector** as a fast-follow over the existing `packages/mcp` server. Also expanded the agent roster to a full delivery team (9 lead/role agents) and recorded the build phasing (single-turn → streaming chat → tool actions → MCP).
- 2026-06-20 — Shipped a draggable dashboard, Pomodoro, docs/wiki, and GitHub integration together on one branch (migration `0007`) — user asked for all four. Dashboard/Pomodoro and docs are pulled forward from the continuous-delivery backlog / Phase 4; GitHub PR↔task linking is the Phase 2 wedge. Decisions made while building: (1) GitHub stays **inbound-only** for now — an HMAC-verified webhook drives PR↔task links and status, with **no stored access token**, so there's no encrypted-secret-at-rest requirement yet (outbound API / posting back to PRs is deferred); (2) the webhook resolves connections by `(owner, repo)` across workspaces via the table-owner (RLS-bypassing) connection, same precedent as invite/session lookups, then re-enters `withWorkspace` per verified connection for all writes; (3) GitHub events are **not** written to the `activity` feed — its CHECK constraint rejects new types, and widening it is a separate database-engineer change; (4) docs ship as a **single-writer** Markdown base with a dependency-free, HTML-free renderer (no Yjs/tiptap yet — co-editing stays deferred per §2); (5) dashboard completion stats use `tasks.updatedAt` as a proxy until a `completedAt` column exists. Built by three specialists in parallel on disjoint files over a shared schema/nav foundation.
